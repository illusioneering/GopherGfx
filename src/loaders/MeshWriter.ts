...
{
    static saveOBJ(filename: string, mesh: Mesh): void
    {
    ...
    static savePLY(filename: string, mesh: Mesh, binary = true): void
    {
        const vertices = mesh.getVertices();
        const normals = mesh.getNormals();
        const colors = mesh.getColors();
    ...
    static saveGLTF(filename: string, transform: Transform3): void
    {
        const doc = MeshWriter.createGLTF(transform);
        MeshWriter.writeJSON(filename, doc); 
    }

    static saveGLB(filename: string, transform: Transform3): void
    {
        const doc = MeshWriter.createGLTF(transform);
        const io = new WebIO();
    ...
    private static createGLTF(transform: Transform3): Document
    {
    ...
    private static createGLTFRecursive(transform: Transform3, doc: Document, buffer: Buffer, node: Node)
    {
